...
int main( int argc, char *argv[] )
{
  ...
  Mat element = getStructuringElement(
      MORPH_ELLIPSE, Size(2 * erosion_size + 1, 2 * erosion_size + 1),
  ...
  workingimage.convertTo(
      lowdepth, CV_8U, 255.0 / (maxVal - minVal),
  ...
  lowdepth.convertTo(
      lowdepth, CV_16U, 65535.0 / (maxVal - minVal),
      -minVal * 65535.0 / (maxVal - minVal));
  mask.convertTo(
      mask, CV_16U, 65535.0 / (maxVal - minVal),
      -minVal * 65535.0 / (maxVal - minVal));
  invertmask.convertTo(
      invertmask, CV_16U, 65535.0 / (maxVal - minVal),
      -minVal * 65535.0 / (maxVal - minVal));

  //  Subtract bad pixels from original image and add new pixels
